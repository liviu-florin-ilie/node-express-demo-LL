...
```
tanzu apps workload create node-express-demo-LL -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/node-express-demo-LL-source \
  --type web
```
...
```
tanzu apps workload get node-express-demo-LL
```
